...
/*function searchJob(work, geo) {
    let job = work.toLowerCase(); //conversione dei parametri in stringhe
    ...
    for (let element of jobs) {
      ...
      let title = element.location.toLocaleLowerCase();

      if (location.includes(loc) && title.includes(job)) {

        delete element.description; //eliminazione dati non voluti
...
const search = function (posL, posG) {
    const result = [];
    const keyToBeDeleted = ["description", "requirements", "benefits", "company_profile"];
    for (let job of jobs) {
        if (job.title.toLocaleLowerCase().includes(posL.toLocaleLowerCase()) && job.location.toLocaleLowerCase().includes(posG.toLocaleLowerCase())) {
            for (let key of keyToBeDeleted) {
                if (key in copyObj) {
                    delete copyObj[key];
                }
            }
